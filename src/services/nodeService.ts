...
export class NodeService {
    ...
    static setWebSocketManager(wsManager: WebSocketManager) {
    ...
    static createKey(servername: string) {
        try {
            // 先检查该 servername 是否在线，在线就踢掉
            this.wsManager.kickNodeByServername(servername);
    ...
    static deleteKey(servername: string) {
        try {
            this.wsManager.kickNodeByServername(servername);
    ...
    static kickNode(servername: string) {
        try {
            const result = this.wsManager.kickNodeByServername(servername);
            if (result) {
